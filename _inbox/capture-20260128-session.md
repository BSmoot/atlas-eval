# Capture: 2026-01-28 Session

## High Priority (6)

| # | Item | Strand | Owner/With |
|---|------|--------|------------|
| 1 | Ops report structure and measures | WS-008 | CN/AR/ASt/DC → KW/KM/KP/CD |
| 2 | 2026 Video | WS-004 | — |
| 3 | Performance Metrics for 2026 - review and adjust | WS-008 | — |
| 4 | Escalation Log with Ops | WS-003 | Ops |
| 5 | Program owner performance metrics | WS-003 | AS |
| 6 | Business ops performance metrics and release/change mgmt tuning (simplification) | WS-003 | — |

---

## By Category

### HR / Admin (6)
| # | Item | Status | Strand |
|---|------|--------|--------|
| 6 | Patty - 2x paperwork and CT confirmation | ready | — |
| 7 | Office manager search and building setup | ready | — |
| 8 | Aux to FTE transition planning | ready | WS-007 |
| 9 | Clear all-level cascade requirements | ready | WS-002 |
| 10 | Paylocity FSA receipts | quick | — |
| 11 | Sucuri transfer to Darrell | quick | — |

### Programs / Execution (7)
| # | Item | Status | Strand |
|---|------|--------|--------|
| 12 | Program 3 catalogue and low hanging fruit | ready | WS-003 |
| 13 | Program cadence meetings | ready | WS-003 |
| 14 | Reporting request - data catalogues in program 4 | ready | WS-003 |
| 15 | All hands visibility and topics confirmed (program 5) | ready | WS-003 |
| 16 | Launch cadence and requirements | ready | WS-003 |
| 17 | Strategy & hindsight agenda and schedule | ready | WS-003 |
| 62 | **HIGH** Business ops performance metrics and release/change mgmt tuning (simplification) | ready | WS-003 |

### AI / Tech (9)
| # | Item | Status | Strand |
|---|------|--------|--------|
| 18 | AI Innovation and IRIS team and scope alignment | ready | WS-001 |
| 19 | Share Lazard w/ tech/ai team and ELT | ready | WS-004 |
| 20 | AI launch plan and targets | ready | WS-001 |
| 21 | End to end demo for AI | ready | WS-001 |
| 22 | AI department | ready | WS-006 |
| 23 | CX and Sales release prep and alignment - tech and insights | ready | WS-001 |
| 24 | Claude agent cleanup and naming | ready | WS-006 |
| 25 | S code augmentation follow up - seav | ready | WS-001 |
| 26 | Decision assist build | ready | WS-001 |

### Sales / Revenue (8)
| # | Item | Status | Strand |
|---|------|--------|--------|
| 27 | CRO feedback to True | ready | WS-007 |
| 28 | Aux-Product with Sales Enablement and then Sales | ready | WS-007 |
| 29 | Brad approach and accounts | ready | WS-007 |
| 30 | Ryan and VBT convo | ready | WS-007 |
| 31 | Playbook use tracking in SFDC | ready | WS-007 |
| 32 | Revenue risk read-in and action | ready | WS-007 |
| 33 | Growth testing resurfaced - 251112 | ready | WS-007 |
| 34 | Video creation, case studies and bounty for AI participation | ready | WS-004 |

### Product / Positioning (4)
| # | Item | Status | Strand |
|---|------|--------|--------|
| 35 | Product approach for non-technical offerings | ready | WS-004 |
| 36 | Case study ownership | ready | WS-004 |
| 37 | Horizontal-vertical research | ready | WS-004 |
| 38 | Product-tech measures | ready | WS-008 |

### Board / Investor (2)
| # | Item | Status | Strand |
|---|------|--------|--------|
| 39 | Board deck prep | ready | WS-009 |
| 40 | Ops support for CX | ready | WS-003 |

### People / 1:1s (7)
| # | Item | Status | Strand |
|---|------|--------|--------|
| 41 | Customer council interviews | ready | WS-007 |
| 42 | Staff interviews | ready | WS-002 |
| 43 | Rafferty update | ready | WS-001 |
| 44 | RJ review | ready | WS-001 |
| 45 | Leo Update | ready | WS-001 |
| 46 | Volunteers to screen record/interview capture | ready | WS-001 |
| 47 | AP check-in topics | ready | — |

### Research (4)
| # | Item | Status | Strand |
|---|------|--------|--------|
| 48 | CIO research | ready | WS-004 |
| 49 | AEO research | ready | WS-004 |
| 50 | 250729_Anders and 250818_ToDo | backlog | — |
| 51 | Develop-Sustain-Improve in use for measures | ready | WS-008 |

### Misc (1)
| # | Item | Status | Strand |
|---|------|--------|--------|
| 52 | Upsthackathon plan | ready | WS-001 |

### CT / COO Alignment (2)
| # | Item | Status | Strand |
|---|------|--------|--------|
| 53 | AI research share w/ CT - cadence and regular update approach | ready | WS-001 |
| 54 | AI intent and expectation discovery w/ CT - compare to business/program status for COO | ready | WS-001 |

### AI Research (7)
| # | Item | Status | Strand |
|---|------|--------|--------|
| 55 | Synthesis research applied | ready | WS-001 |
| 56 | Phone claude items added to word doc on AI threads | ready | WS-001 |
| 57 | NLP vs AI mapping added to claude | ready | WS-001 |
| 58 | Engineering requirements spec added to apex | ready | WS-006 |
| 59 | Research on claude-code setups for teams | ready | WS-006 |
| 60 | Research on product-launch patterns that are successful | ready | WS-004 |
| 61 | Checking dynamically for verification rules for our claude hook | ready | WS-006 |

---

## Summary

- **Total captured:** 62 items
- **High priority:** 6
- **Quick tasks:** 2
- **Ready:** 44
- **Backlog:** 1
- **No strand:** 6

## Strand Mapping (Confirmed)

| Strand | Count | Key Items |
|--------|-------|-----------|
| WS-001 (AI Exploration) | 18 | AI launch, IRIS alignment, demo, decision assist, CX/Sales release, S code, volunteers, hackathon, Rafferty, RJ, Leo, CT alignment, synthesis research, NLP mapping |
| WS-002 (Vision/Transform) | 2 | Cascade requirements, staff interviews |
| WS-003 (Program Stewardship) | 9 | Program cadence, P3 catalogue, escalation log, program metrics, all hands, launch cadence, strategy & hindsight, ops support, biz ops metrics |
| WS-004 (Strategic Positioning) | 10 | 2026 Video, non-technical product, Lazard share, video/case studies, case ownership, h-v research, CIO/AEO research, product-launch patterns |
| WS-006 (Agentic Org) | 5 | AI department, claude agent cleanup, apex eng spec, claude-code teams research, verification hooks |
| WS-007 (Revenue Engine) | 10 | CRO feedback, Aux-Product, revenue risk, Aux-FTE, Brad, Ryan/VBT, playbook tracking, growth testing, customer council |
| WS-008 (Strategic Insights) | 4 | Ops report structure, performance metrics, product-tech measures, DSI measures |
| WS-009 (Investor Alignment) | 1 | Board deck prep |
| No strand | 6 | Patty, Paylocity, office manager, Sucuri, AP check-in, old Anders/ToDo |

---

*Captured: 2026-01-28*
*Mappings confirmed: 2026-01-28*
